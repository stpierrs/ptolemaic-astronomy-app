...
import {
  ...
  weatherFromChart, fortnightForecast,
...
import { solarReturnDate, solarReturnChart } from '../core/solarReturn.js';
import { heliacalPhase, PHASE_LABEL, PHASE_GLYPH } from '../core/heliacal.js';
import { buildNatalReport } from '../core/natalReport.js';
import { buildReport as buildReportV2 } from '../core/reportGenerator.js';
import { VERDICT_BADGE } from '../core/verdictLanguage.js';
import {
  exportJson, exportSvg, exportPng, exportPdf, exportShareCard,
  downloadText, downloadDataUrl, downloadBlob,
} from '../core/chartExport.js';
...
import { maybeRunAstrologyOnboarding, openAstrologyOnboarding } from './astrologyOnboarding.js';
import { computeTransits } from '../core/transitEngine.js';
...
export function buildAstrologyApp(model) {
  ...
  app.innerHTML = `
    <div class="pa-header">
      ...
      <button class="pa-btn pa-newchart" id="pa-new-chart-btn" type="button" title="Cast a new nativity">+ New chart</button>
      <nav class="pa-nav">
        ...
        <button class="pa-tab-btn" data-tab="ephemeris" type="button">📊 Ephemeris</button>
    ...
    <div class="pa-body">
      ...
      <div class="pa-tab" id="pa-tab-transits" style="display:none">
        <div id="pa-transitsv2-content"></div>
      </div>
      <div class="pa-tab" id="pa-tab-profections" style="display:none">
        <div id="pa-profectionsv2-content"></div>
      </div>
      ...
      <div class="pa-tab" id="pa-tab-ephemeris" style="display:none">
        <div id="pa-ephemeris-content"></div>
      </div>
  ...
  const tabs = {
    ...
    ephemeris:   app.querySelector('#pa-tab-ephemeris'),
  };
  ...
  function switchTab(name) {
    ...
    if (name === 'transits')    renderTransitsV2();
    if (name === 'profections') renderProfectionsV2();
    if (name === 'lifespan')    renderLifespan();
    if (name === 'mundane')     renderMundane();
    if (name === 'returns')     renderReturns();
    if (name === 'report')      renderReport();
    if (name === 'ephemeris')   renderEphemerisTab();
  }
  ...
  // ── "+ New chart" button — opens the Guide 09 onboarding modal manually.
  // Provides the path back to chart creation that the auto-fire-once trigger
  // doesn't offer (e.g. after a dismissed first-launch flow, or for a second
  // chart in addition to the primary).
  app.querySelector('#pa-new-chart-btn')?.addEventListener('click', async () => {
    try {
      const rec = await openAstrologyOnboarding({ markPrimary: false });
      if (rec) {
        await refreshProfilesCache();
        if (rec.chart) {
          ...
          const tEl = app.querySelector('#pa-today-panel');
          if (tEl) { tEl.dataset.rendered = ''; _todayLastRender = 0; }
          try { renderTodayPanel(); } catch (_) {}
        }
      }
    } catch (e) {
      console.error('[astrology] new chart failed:', e);
    }
  });
...
`;
  ...
  function renderSect() {
    ...
    if (!currentChart) {
      el.innerHTML = '<div style="padding:20px;color:var(--muted)">Draw a chart first — tap <strong>+ New chart</strong> in the header.</div>';
      return;
    }
  ...
  // ── Profections tab v2 (Guide 11) — annual profection + life-timeline ────
  function renderProfectionsV2() {
    const el = app.querySelector('#pa-profectionsv2-content');
    if (!el) return;
    if (!currentChart) {
      el.innerHTML = '<div style="padding:20px;color:var(--muted)">Draw a chart first — tap <strong>+ New chart</strong> in the header.</div>';
      return;
    ...
    const ch  = currentChart;
    const now = new Date();
    const ageF = (now - ch.date) / (365.25 * 86400000);
    const age  = Math.max(0, Math.floor(ageF));
    const ascSignIdx = Math.floor((((ch.ascLon % 360) + 360) % 360) / 30);
    const profHouse  = ((age % 12) + 12) % 12 + 1;
    const profSignIdx = (ascSignIdx + (age % 12)) % 12;

    const SIGN_NAMES = ['Aries','Taurus','Gemini','Cancer','Leo','Virgo','Libra','Scorpius','Sagittarius','Capricornus','Aquarius','Pisces'];
    const SIGN_SYMS  = ['♈','♉','♊','♋','♌','♍','♎','♏','♐','♑','♒','♓'];
    const lordOfSign = (idx) => {
      for (const [planet, signs] of Object.entries(DOMICILE)) if (signs.includes(idx)) return planet;
      return null;
    };
    const lord = lordOfSign(profSignIdx);

    // Life timeline (0..90) — colour each year cell by its profected sign's lord.
    const LIFE_W = 800;
    const totalYears = 90;
    const cellW = LIFE_W / totalYears;
    let timeline = '';
    for (let a = 0; a < totalYears; a++) {
      const si = (ascSignIdx + (a % 12)) % 12;
      const ld = lordOfSign(si);
      const col = PLANET_COLORS[ld] || '#666';
      const x = a * cellW;
      const opacity = a === age ? 1 : 0.55;
      timeline += `<rect x="${x}" y="0" width="${cellW - 0.5}" height="20" fill="${col}" opacity="${opacity}"></rect>`;
      if (a % 12 === 0) timeline += `<text x="${x + 2}" y="34" font-size="9" fill="#888">${a}</text>`;
    }
    timeline += `<line x1="${age * cellW + cellW/2}" y1="-3" x2="${age * cellW + cellW/2}" y2="38" stroke="#ffd060" stroke-width="1.5"></line>`;

    // 12-spoke SVG profection wheel — highlight the active sign sector.
    const wheelHtml = renderProfectionWheelSvgInline(ch, age);
    ...
    el.innerHTML = `
      <div style="margin-bottom:14px;padding:14px;background:rgba(212,184,120,0.08);border:1px solid rgba(212,184,120,0.2);border-radius:4px">
        <div style="font-size:11px;color:var(--muted);margin-bottom:6px">Annual Profection · Age ${age}</div>
        <div style="display:flex;gap:20px;flex-wrap:wrap;align-items:center">
          <div>
            <div style="font-size:11px;color:var(--muted)">Activated House</div>
            <div style="font-size:22px;color:#ffd060">House ${profHouse}</div>
            <div style="font-size:11px;color:var(--muted)">${HOUSE_MEANINGS[profHouse - 1]?.name || ''}</div>
          </div>
          <div>
            <div style="font-size:11px;color:var(--muted)">Activated Sign</div>
            <div style="font-size:22px;color:#ccc">${SIGN_NAMES[profSignIdx]} ${SIGN_SYMS[profSignIdx]}</div>
          </div>
          <div>
            <div style="font-size:11px;color:var(--muted)">Lord of the Year</div>
            <div style="font-size:22px;color:${PLANET_COLORS[lord] || '#aaa'}">${PLANET_SYMBOLS[lord] || '?'} ${lord ? lord.charAt(0).toUpperCase() + lord.slice(1) : '—'}</div>
          </div>
        </div>
      </div>

      <div style="color:var(--accent);font-size:12px;font-family:${GK_FONT};margin-bottom:6px">Profection wheel</div>
      <div>${wheelHtml}</div>

      <div style="color:var(--accent);font-size:12px;font-family:${GK_FONT};margin:16px 0 6px">Life timeline (0–90)</div>
      <svg viewBox="0 0 ${LIFE_W} 42" style="width:100%">${timeline}</svg>
  ...
  // SVG profection wheel: outline highlights the activated sign sector.
  function renderProfectionWheelSvgInline(chart, age) {
    const size = 380;
    // Reuse renderChartWheelSvg, then overlay a thick gold sector on the
    // active sign. Easier to draw the overlay as a path here on a copy.
    const wheel = renderChartWheelSvg(chart, { size });
    const ascSignIdx = Math.floor((((chart.ascLon % 360) + 360) % 360) / 30);
    const activeSignIdx = (ascSignIdx + (age % 12)) % 12;
    const startLon = activeSignIdx * 30;
    // CX/CY = 300 in viewBox 600. R_OUTER = 280 / R_ZODIAC = 240 (matches svg renderer)
    const CX = 300, CY = 300, R_OUT = 280, R_IN = 240;
    const polar = (r, deg) => {
      const rad = deg * Math.PI / 180;
      return [CX - r * Math.cos(rad), CY + r * Math.sin(rad)];
    };
    const a1 = ((startLon - chart.ascLon) % 360 + 360) % 360;
    const a2 = a1 + 30;
    const [x1o, y1o] = polar(R_OUT, a1);
    const [x2o, y2o] = polar(R_OUT, a2);
    const [x1i, y1i] = polar(R_IN, a1);
    const [x2i, y2i] = polar(R_IN, a2);
    const d = `M ${x1o} ${y1o} A ${R_OUT} ${R_OUT} 0 0 0 ${x2o} ${y2o} ` +
              `L ${x2i} ${y2i} A ${R_IN} ${R_IN} 0 0 1 ${x1i} ${y1i} Z`;
    const SVG_NS = 'http://www.w3.org/2000/svg';
    const path = document.createElementNS(SVG_NS, 'path');
    path.setAttribute('d', d);
    path.setAttribute('fill', 'none');
    path.setAttribute('stroke', '#ffd060');
    path.setAttribute('stroke-width', '3');
    path.setAttribute('opacity', '0.95');
    wheel.appendChild(path);
    return wheel.outerHTML;
  }

  // ── Transits tab v2 (Guide 10) — engine-driven cards + SVG timeline ───────
  function renderTransitsV2() {
    const el = app.querySelector('#pa-transitsv2-content');
    if (!el) return;
    if (!currentChart) {
      el.innerHTML = '<div style="padding:20px;color:var(--muted)">Draw a chart first — tap <strong>+ New chart</strong> in the header.</div>';
      return;
    ...
    let active = [], upcoming = [];
    try {
      const result = computeTransits(currentChart, today);
      active = result.active; upcoming = result.upcoming;
    } catch (e) {
      console.warn('[transits] compute failed:', e);
      el.innerHTML = '<div style="color:#ff7055;padding:20px">Transit computation failed — see console.</div>';
      return;
    }

    const aspectGlyph = (name) => ({
      Conjunction:'☌', Sextile:'⚹', Square:'□', Trine:'△', Opposition:'☍',
    })[name] || '';
    const cap = s => s ? s.charAt(0).toUpperCase() + s.slice(1) : '';

    const transitCardHtml = (t) => {
      const tCol = PLANET_COLORS[t.transit_planet] || '#aaa';
      const nCol = PLANET_COLORS[t.natal_planet]   || '#aaa';
      const qDesc = t.quality_interaction?.description || '';
      const startStr = t.start_date ? t.start_date.toISOString().slice(0, 10) : '—';
      const endStr   = t.end_date   ? t.end_date.toISOString().slice(0, 10)   : '—';
      const stations = (t.stations || []).map(s =>
        `<span class="pa-transit-station">${s.type === 'retrograde' ? '℞' : 'D'} ${s.date.toISOString().slice(5,10)}</span>`
      ).join('');
      const pct = (t.strength * 100).toFixed(0);
      return `<div class="pa-transit-card">
        <div class="pa-transit-headline">
          <span style="color:${tCol}">${PLANET_SYMBOLS[t.transit_planet]} ${cap(t.transit_planet)}</span>
          <span>${aspectGlyph(t.aspect_type)} ${t.aspect_type}</span>
          <span style="color:${nCol}">${PLANET_SYMBOLS[t.natal_planet]} natal ${cap(t.natal_planet)}</span>
          <span class="pa-strength" style="margin-left:auto">${pct}%</span>
        </div>
        ${qDesc ? `<div class="pa-transit-quality">${qDesc}</div>` : ''}
        <div class="pa-transit-meta">
          ${startStr} → ${endStr}
          ${stations ? ' · ' + stations : ''}
          ${t.applying ? ' · applying' : ' · separating'}
          ${(t.degreeOffExact != null) ? ` · ${t.degreeOffExact.toFixed(2)}° off exact` : ''}
        </div>
      </div>`;
    };

    const timelineHtml = renderTransitTimeline(
      active.filter(t => t.start_date && t.end_date).slice(0, 12)
    );

    const upcomingHtml = upcoming.slice(0, 10).map(t => {
      const days = Math.round((t.start_date - today) / 86400000);
      return `<div class="pa-aspect-row">
        <span style="font-size:11px;color:var(--muted);min-width:64px">${t.start_date.toISOString().slice(0,10)}</span>
        <span style="font-size:11px;color:var(--muted);min-width:42px">+${days}d</span>
        <span style="color:${PLANET_COLORS[t.transit_planet]||'#aaa'}">${PLANET_SYMBOLS[t.transit_planet]}</span>
        <span style="font-size:11px">${aspectGlyph(t.aspect_type)} ${t.aspect_type}</span>
        <span style="color:${PLANET_COLORS[t.natal_planet]||'#aaa'}">${PLANET_SYMBOLS[t.natal_planet]}</span>
        <span style="font-size:10px;color:var(--muted);flex:1">natal ${cap(t.natal_planet)}</span>
      </div>`;
    }).join('') || '<div style="color:var(--muted);font-size:11px;padding:6px">No new transits in the next 12 months.</div>';

    el.innerHTML = `
      <div style="margin-bottom:14px;padding:12px;background:rgba(212,184,120,0.08);border:1px solid rgba(212,184,120,0.2);border-radius:4px">
        <div style="font-size:11px;color:var(--muted);margin-bottom:4px">Transits — ${today.toLocaleDateString('en-US',{year:'numeric',month:'long',day:'numeric'})}</div>
        <div style="font-size:11px;color:var(--muted);font-style:italic">Strength = essential dignity × aspect weight × phase × retrograde × natal resilience.</div>
      </div>

      <div style="color:var(--accent);font-size:12px;font-family:${GK_FONT};margin-bottom:6px">Top active transits</div>
      <div style="margin-bottom:14px">${active.slice(0, 8).map(transitCardHtml).join('') || '<div style="color:var(--muted)">None.</div>'}</div>

      <div style="color:var(--accent);font-size:12px;font-family:${GK_FONT};margin-bottom:6px">Timeline (±18 months)</div>
      ${timelineHtml}

      <div style="color:var(--accent);font-size:12px;font-family:${GK_FONT};margin:14px 0 6px">Upcoming (next 12 months)</div>
      ${upcomingHtml}
    `;
  }

  function renderTransitTimeline(transits) {
    if (!transits.length) return '<div style="color:var(--muted);font-size:11px;padding:6px">No bars in current window.</div>';
    const today = new Date();
    const MS_DAY_ = 86400000;
    const windowStart = today.getTime() - 6  * 30 * MS_DAY_;
    const windowEnd   = today.getTime() + 18 * 30 * MS_DAY_;
    const span = windowEnd - windowStart;
    const W = 800;
    const xLabelW = 130;
    const rowH = 28;
    const totalH = transits.length * rowH + 36;
    const colour = {
      Conjunction:'#ffd060', Sextile:'#4a7fb5', Square:'#c54040',
      Trine:'#c8a84b', Opposition:'#888',
    };
    const aspectGlyph = (name) => ({
      Conjunction:'☌', Sextile:'⚹', Square:'□', Trine:'△', Opposition:'☍',
    })[name] || '';
    const rows = transits.map((t, i) => {
      const sMs = Math.max(t.start_date.getTime(), windowStart);
      const eMs = Math.min(t.end_date.getTime(),   windowEnd);
      if (eMs <= sMs) return '';
      const x1 = xLabelW + ((sMs - windowStart) / span) * (W - xLabelW);
      const x2 = xLabelW + ((eMs - windowStart) / span) * (W - xLabelW);
      const col = colour[t.aspect_type] || '#888';
      const y = 22 + i * rowH;
      const label = `${t.transit_planet} ${aspectGlyph(t.aspect_type)} ${t.natal_planet}`;
      return `
        <text x="6" y="${y + 5}" font-size="10" fill="#888">${label}</text>
        <rect x="${x1}" y="${y - 8}" width="${Math.max(2, x2 - x1)}" height="16"
              fill="${col}" opacity="0.6"></rect>`;
    }).join('');
    const todayX = xLabelW + ((today.getTime() - windowStart) / span) * (W - xLabelW);
    return `<svg viewBox="0 0 ${W} ${totalH}" style="width:100%;background:rgba(0,0,0,0.2);border-radius:4px">
      <line x1="${todayX}" y1="14" x2="${todayX}" y2="${totalH - 6}" stroke="#ffd060" stroke-dasharray="4,3"></line>
      <text x="${todayX + 4}" y="14" font-size="10" fill="#ffd060">today</text>
      ${rows}
    </svg>`;
  }
  ...
  function renderMundane() {
    ...
    // 14-day weather forecast — independent of currentChart (uses live Moon).
    let forecast = [];
    try { forecast = fortnightForecast(new Date(), source); } catch (_) {}
    ...
    const eclipseRows = eclipses.map(e => {
      ...
      const glyph = e.signGlyph || SIGN_SYMS[e.signIdx] || '';
      return `
        <div class="pa-aspect-row" style="padding:8px;border-bottom:1px solid rgba(255,255,255,0.05)">
          <span style="font-size:11px;color:var(--muted);min-width:90px">${e.dateLabel || (e.when ? e.when.toISOString().slice(0,10) : '')}</span>
          <span style="font-size:12px;color:#ccc;min-width:84px">${kindLabel} ${e.type || ''}</span>
          <span style="font-size:14px;color:#ffd060;min-width:80px">${glyph} ${e.sign}</span>
    ...
    const conjunctionRows = conjunctions.map(c => `
      <div class="pa-aspect-row">
        ...
        <span style="font-size:12px;color:#ccc">${SIGN_SYMS[c.signIdx] || ''} ${c.sign}</span>
    ...
    const weatherRow = weather
      ? `<div style="font-size:13px;color:#ccc">${weather.label}</div>
         <div style="font-size:10px;color:var(--muted);margin-top:4px">Moon in ${weather.moonSignGlyph || ''} ${weather.moonSign} · ${weather.primary} / ${weather.secondary}</div>`
      : '<div style="color:var(--muted);font-size:11px">Draw a chart to see the weather verdict.</div>';

    const forecastRows = forecast.length
      ? forecast.map(w => `<div class="pa-aspect-row" style="font-size:11px">
          <span style="color:var(--muted);min-width:62px">${w.date.toISOString().slice(5,10)}</span>
          <span style="min-width:64px">${w.moonSignGlyph} ${w.moonSign}</span>
          <span style="color:var(--muted);min-width:96px">${w.primary} / ${w.secondary}</span>
          <span style="color:#ccc;flex:1">${w.label}</span>
        </div>`).join('')
      : '<div style="color:var(--muted);font-size:11px;padding:6px">Forecast unavailable.</div>';
    ...
    el.innerHTML = `
      ...
      <div style="color:var(--accent);font-size:12px;font-family:${GK_FONT};margin-bottom:6px">Seasonal Quality Forecast — next 14 days</div>
      <div style="margin-bottom:14px">${forecastRows}</div>
  ...
  function renderReturns() {
    ...
    el.innerHTML = `
      ...
      <div class="pa-wheel-toggle" style="display:flex;gap:6px;margin-bottom:8px;flex-wrap:wrap">
        ...
        <button class="pa-btn" id="pa-export-btn" style="font-size:11px;padding:3px 8px;margin-left:auto" type="button">Export ▾</button>
      </div>
    ...
    // Guide 16 — Export ▾ menu. Operates on the natal chart (not the
    // return chart) — Returns is where the user most often wants to save.
    el.querySelector('#pa-export-btn')?.addEventListener('click', async () => {
      if (!currentChart) return;
      const choice = await chooseExportMenu();
      if (!choice) return;
      const stem = (currentChart.meta?.name || currentChart._name || 'chart')
        .replace(/[^a-z0-9_-]/gi, '_');
      try {
        if (choice === 'JSON')       downloadText(exportJson(currentChart), `${stem}.json`);
        else if (choice === 'SVG')   downloadText(exportSvg(currentChart),  `${stem}.svg`, 'image/svg+xml');
        else if (choice === 'PNG')   downloadDataUrl(await exportPng(currentChart, 1200),  `${stem}.png`);
        else if (choice === 'PDF') {
          const blob = await exportPdf(currentChart);
          if (blob) downloadBlob(blob, `${stem}.pdf`);
        } else if (choice === 'Share card') {
          downloadDataUrl(await exportShareCard(currentChart), `${stem}_share.png`);
        }
      } catch (err) {
  ...
  // Choose-from-menu helper (PART 24 step 3): a centred overlay with 5
  // options. Click outside to cancel. Returns the choice string or null.
  function chooseExportMenu() {
    return new Promise((resolve) => {
      const overlay = document.createElement('div');
      overlay.className = 'pa-menu-overlay';
      const opts = ['JSON', 'SVG', 'PNG', 'PDF', 'Share card'];
      overlay.innerHTML = `
        <div class="pa-menu-box">
          <div class="pa-menu-title">Export chart</div>
          ${opts.map(o => `<button class="pa-btn pa-menu-opt">${o}</button>`).join('')}
          <button class="pa-btn pa-menu-cancel">Cancel</button>
        </div>`;
      overlay.addEventListener('click', (ev) => {
        if (ev.target.matches('.pa-menu-opt')) {
          resolve(ev.target.textContent);
          overlay.remove();
        } else if (ev.target === overlay || ev.target.matches('.pa-menu-cancel')) {
          resolve(null);
          overlay.remove();
        }
      });
  ...
  // ── Ephemeris tab (Guide 14) ───────────────────────────────────────────────
  // Mirror of the astronomy HUD's data, rendered inside the astrology view so
  // a user inside Astrology doesn't have to leave to see today's positions.
  // ── Today panel (Guide 17 — daily-engagement loop) ────────────────────────
  // Sits at the top of the astrology body, above the tab nav content. Four
  // micro-rows: Sky now (Moon + quality), top active transit (if a chart
  // is loaded), current profected house, and a brief upcoming-events feed.
  let _todayLastRender = 0;
  function renderTodayPanel() {
    const el = app.querySelector('#pa-today-panel');
    if (!el) return;
    const now = Date.now();
    if (now - _todayLastRender < 5 * 60 * 1000 && el.dataset.rendered) return;
    _todayLastRender = now;
    el.dataset.rendered = '1';
    ...
    let moonRow = '';
    try {
      const moonInfo = getPlanetLocalSafe('moon', today, source);
      if (moonInfo) {
        const q = signQualityLocalSafe(moonInfo.sign_index);
        moonRow = `<div class="pa-today-row">
          <strong>Sky now:</strong>
          ☽ ${moonInfo.sign_glyph} ${moonInfo.sign} ${moonInfo.degree_in_sign.toFixed(0)}°
          <span style="color:var(--muted)">(${q.primary}/${q.secondary})</span>
        </div>`;
      }
    } catch (_) {}
    ...
    if (currentChart) {
      try {
        const { active } = computeTransits(currentChart, today);
        const t = active[0];
        if (t) {
          topTransitRow = `<div class="pa-today-row">
            <strong>Active transit:</strong>
            ${t.transit_planet} ${t.aspect_type} natal ${t.natal_planet}
            <span style="color:#ffd060">${(t.strength * 100).toFixed(0)}%</span>
          </div>`;
        }
      } catch (_) {}
    }
    ...
    let profRow = '';
    if (currentChart && typeof currentChart.ascLon === 'number') {
      const age = Math.max(0, Math.floor((today - currentChart.date) / (365.25 * 86400000)));
      const profHouse = ((age % 12) + 12) % 12 + 1;
      profRow = `<div class="pa-today-row">
        <strong>Profected house:</strong> H${profHouse}
        <span style="color:var(--muted)">(age ${age})</span>
      </div>`;
    }

    let eventsHtml = '';
    try {
      const events = upcomingEvents(today, 7, source).slice(0, 4);
      if (events.length) {
        eventsHtml = `<div class="pa-today-events">
          ${events.map(e => `<div>
            ${e.date.toISOString().slice(5,10)} · ${PLANET_SYMBOLS[e.body] || ''} ${e.body} ${e.detail}
          </div>`).join('')}
        </div>`;
      }
    } catch (_) {}
  ...
  // Local shims that defer to already-loaded modules — avoids extra top-level
  // imports for this single panel.
  function getPlanetLocalSafe(name, date, source) {
    try {
      // Re-use computeFullChart's adapter path for hot caching.
      const tmp = computeFullChart(date, currentChart?.lat ?? 0, currentChart?.lon ?? 0, source);
      const p = tmp.astronomy?.positions?.[name];
      return p ? {
        sign:           p.sign,
  ...
  function signQualityLocalSafe(idx) {
    const SIGN = [
      ['hot','dry'],['cold','dry'],['hot','moist'],['hot','moist'],
      ['hot','dry'],['cold','dry'],['cold','moist'],['cold','moist'],
      ['hot','dry'],['cold','dry'],['cold','moist'],['cold','moist'],
    ];
    const [primary, secondary] = SIGN[((idx % 12) + 12) % 12];
    return { primary, secondary };
  }

  function renderEphemerisTab() {
    const el = app.querySelector('#pa-ephemeris-content');
    if (!el) return;
    const today  = new Date();
    const source = currentChart?.source || (model.state?.BodySource || 'ibnshatir');
    const isDay  = (model.computed?.SunAltDeg ?? 1) > 0;

    let positions;
    try { positions = getAllPositionsLocal(today, source); }
    catch (e) {
      console.warn('[ephemeris] positions failed:', e);
      el.innerHTML = '<div style="padding:20px;color:#ff7055">Ephemeris unavailable — see console.</div>';
      return;
    }

    const dignityClass = (dig) => {
      if (dig.domicile)        return 'dig-dom';
      if (dig.exaltation)      return 'dig-exalt';
      if (dig.triplicity)      return 'dig-tri';
      if (dig.term)            return 'dig-term';
      if (dig.face)            return 'dig-face';
      if (dig.detriment)       return 'dig-det';
      if (dig.fall)            return 'dig-fall';
      return 'dig-per';
    };

    const rows = Object.values(positions).map(p => {
      const dig = getPlanetDignity(p.planet, p.longitude, isDay);
      const cls = dignityClass(dig);
      return `<tr class="${cls}">
        <td>${p.glyph} ${p.planet.charAt(0).toUpperCase() + p.planet.slice(1)}</td>
        <td>${p.sign_glyph} ${p.sign} ${p.degree_in_sign.toFixed(1)}°</td>
        <td>${p.is_retrograde ? '℞' : ''}</td>
        <td style="text-transform:capitalize">${(p.heliacal_state || 'normal').replace(/_/g, ' ')}</td>
        <td>${dig.label} ${dig.score >= 0 ? '+' : ''}${dig.score}</td>
      </tr>`;
    }).join('');

    let events = [];
    try { events = upcomingEvents(today, 14, source); } catch (_) {}
    const eventRows = events.slice(0, 14).map(e => `
      <div class="tracker-event">
        <span class="tracker-event-date">${e.date.toISOString().slice(5,10)}</span>
        <span class="tracker-event-body">${PLANET_SYMBOLS[e.body] || ''} ${e.body}</span>
    ...
    el.innerHTML = `
      <div style="margin-bottom:14px;padding:12px;background:rgba(212,184,120,0.08);border:1px solid rgba(212,184,120,0.2);border-radius:4px">
        <div style="font-size:11px;color:var(--muted)">Planet status — ${today.toISOString().slice(0,10)}</div>
      </div>

      <table class="pa-ephem-table">
        <thead><tr>
          <th>Body</th><th>Position</th><th></th><th>Phase</th><th>Dignity</th>
        </tr></thead>
        <tbody>${rows}</tbody>
      </table>

      <div style="color:var(--accent);font-size:12px;font-family:${GK_FONT};margin:18px 0 6px">Upcoming events (14 days)</div>
      ${eventRows}
    `;
  }

  // Inline shim so the Ephemeris tab can use the adapter without importing
  // it at the top of this very-long file. Lazy import is fine because the
  // adapter is already loaded for other tabs.
  function getAllPositionsLocal(date, source) {
    // Adapter is imported elsewhere via natalChartBuilder; reuse via that
    // path's already-resolved module. Otherwise dynamic-import as fallback.
    try {
      // computeFullChart already calls getAllPositions, so the adapter is
      // hot in the import cache. Direct require not available — use chart
      // for today as a quick path.
      const tmp = computeFullChart(date, currentChart?.lat ?? 0, currentChart?.lon ?? 0, source);
      // Re-shape to match getAllPositions' return: { planetName: PART_1_7 }.
      const out = {};
      for (const p of tmp.planets) {
        out[p.name] = {
          planet:         p.name,
          glyph:          p.symbol,
          longitude:      p.lon,
          sign:           tmp.astronomy?.positions?.[p.name]?.sign ?? '',
          sign_index:     p.sign_index,
          sign_glyph:     tmp.astronomy?.positions?.[p.name]?.sign_glyph ?? '',
          degree_in_sign: p.degree_in_sign,
  ...
  // Renamed: the v1 renderer is preserved below as renderReportLegacy. The
  // active renderer below this stub uses Guide 15's buildReportV2 +
  // verdictLanguage badges.
  ...
  // ── Natal Report tab v2 (Guide 15) — verdict-language version ────────────
  function renderReport() {
    ...
    let report;
    try {
      report = buildReportV2(currentChart, { gender: 'm' });
    } catch (e) {
      console.warn('[report] build failed:', e);
      el.innerHTML = '<div style="padding:20px;color:#ff7055">Report generation failed — see console.</div>';
      return;
    }

    const summaryRows = report.summary.map(s => `
      <div class="pa-summary-row">
        <span class="pa-summary-verdict pa-verdict-${s.verdict}">${VERDICT_BADGE[s.verdict] || ''}</span>
        <span class="pa-summary-key">${s.key}</span>
        <span class="pa-summary-headline">${escHtml(s.headline)}</span>
      </div>
    `).join('');

    // Each section renders the headline + verdict, then its body[] paragraphs.
    // Section keys that carry nested house-topic objects (parents, marriage,
    // death) get those rendered as sub-details for drill-in.
    const renderHouseTopic = (topic, label) => {
      if (!topic) return '';
      return `<details class="pa-report-subsection">
        <summary>
          <span class="pa-section-verdict pa-verdict-${topic.verdict}">${VERDICT_BADGE[topic.verdict] || ''}</span>
          <span class="pa-section-key">${label}</span>
          <span class="pa-section-headline">${escHtml(topic.headline)}</span>
        </summary>
        <div class="pa-section-body">
          ${(topic.body || []).map(p => `<p>${escHtml(p)}</p>`).join('')}
        </div>
      </details>`;
    };

    const body = Object.entries(report.sections).map(([key, sec]) => {
      const nestedHtml =
          key === 'parents'  ? renderHouseTopic(sec.father, 'father — H4') + renderHouseTopic(sec.mother, 'mother — H10')
        : key === 'marriage' ? renderHouseTopic(sec.house7, 'house — H7')
        : key === 'death'    ? renderHouseTopic(sec.house8, 'house — H8')
        : '';
      return `<details class="pa-report-section" ${key === 'foundation' ? 'open' : ''}>
        <summary>
          <span class="pa-section-verdict pa-verdict-${sec.verdict}">${VERDICT_BADGE[sec.verdict] || ''}</span>
          <span class="pa-section-key">${key}</span>
          <span class="pa-section-headline">${escHtml(sec.headline)}</span>
        </summary>
        <div class="pa-section-body">
    ...
    el.innerHTML = `
      <div style="margin-bottom:14px;padding:14px;background:rgba(212,184,120,0.08);border:1px solid rgba(212,184,120,0.2);border-radius:4px">
        <div style="font-size:11px;color:var(--muted);margin-bottom:4px">Tetrabiblos III–IV · Natal Report</div>
        <div style="font-size:13px;color:#ccc">${escHtml(currentChart._name || currentChart.meta?.name || 'Current chart')}${currentChart._place ? ' · ' + escHtml(currentChart._place) : ''}</div>
      </div>

      <div class="pa-report-summary">${summaryRows}</div>
      ${body}
  ...
  function show() {
    ...
    // If chartStore already has charts, hydrate currentChart from the
    // primary so all the chart-dependent tabs (Stars, Transits, Report…)
    // have something to render. Fire-and-forget; UI updates on completion.
    if (!currentChart) {
      (async () => {
        try {
          const prim = await getPrimaryChart();
          if (prim?.chart && !currentChart) {
            currentChart = prim.chart;
            currentChart._name  = prim.birth?.name || '';
            currentChart._place = prim.birth?.location_name || '';
            const tEl = app.querySelector('#pa-today-panel');
            if (tEl) { tEl.dataset.rendered = ''; _todayLastRender = 0; }
            try { renderTodayPanel(); } catch (_) {}
          }
        } catch (e) { console.warn('[astrology] primary chart load:', e); }
      })();
    }
        ...
        if (rec) {
          ...
          refreshProfilesCache().then(() => {
            if (rec.chart) {
              ...
              currentChart._name  = rec.birth?.name || '';
              currentChart._place = rec.birth?.location_name || '';
              // Force re-render of Today panel with the new chart context.
              const tEl = app.querySelector('#pa-today-panel');
              if (tEl) { tEl.dataset.rendered = ''; _todayLastRender = 0; }
              try { renderTodayPanel(); } catch (_) {}
            }
