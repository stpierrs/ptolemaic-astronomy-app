...
function chartFromProfile(p, source = 'epicycle2') {
...
function computeFullChart(date, lat, lon, source = 'epicycle2') {
  const base   = buildNatalChart(date, lat, lon, source);
...
export function buildAstrologyApp(model) {
  ...
  app.innerHTML = `
    <div class="pa-header">
      ...
      <nav class="pa-nav">
        ...
        <button class="pa-tab-btn" data-tab="modernity" type="button">⚡ Modernity</button>
  ...
  function renderTransits() {
    ...
    const source = currentChart.source || 'epicycle2';
  ...
  function renderModernity() {
    ...
    const intro = `
      <div style="padding:14px 20px 10px;border-bottom:1px solid rgba(255,255,255,0.07);margin-bottom:16px">
        <div style="font-size:15px;font-weight:700;color:var(--gold);font-family:var(--font-greek);margin-bottom:8px">The Zodiac: Ptolemy vs. Today</div>
        <div style="font-size:11px;color:var(--muted);line-height:1.75;max-width:820px">
          Ptolemy fixed the twelve zodiac signs as 30° arcs of the <em>tropical</em> year, anchored to the equinoxes and solstices — not to the star constellations.
          The sign names (Aries, Taurus…) were borrowed from nearby star patterns of his era, but the signs themselves are geometry, not stars.
          Due to the 26,000-year <em>precession of the equinoxes</em>, those star patterns have since drifted ~24° away from the signs that bear their names.
          Meanwhile, modern psychology rewrote every sign's meaning from outward fate to inward character — and three new planets (Uranus, Neptune, Pluto)
          displaced Saturn, Jupiter, and Mars from three of their ancient rulerships.
        </div>
      </div>`;
    ...
    el.innerHTML = wheelSection + intro + `<div style="padding:0 4px">${cards}</div>`;
  }
